...
abstract class K_Router {
 ...
 $controllerfile=$this->controller_path.$this->controller.'.php';
    ...
    if (!method_exists($this->controller,$this->action)) 
       $this->request_not_found('Controller method not found: '.$this->action);
//    if (!function_exists($function))
//      $this->request_not_found('Function not found: '.$function);
//    call_user_func_array($function,$this->params);
// Call the $foo->bar() method with 2 arguments
$foo = new $this->controller;
call_user_func_array(array($foo, $this->action), $this->params);
    return $this;
  }
  ...
  function request_not_found($msg) {
    ...
    die('<html><head><title>404 Not Found</title></head><body><h1>Not Found</h1><p> '.$msg.'<p>The requested URL was not found on this server.</p><p>Please go <a href="javascript: history.back(1)">back</a> and try again.</p><hr /><p>Powered By: <a href="http://krititech.com">K-MVC</a></p></body></html>');
  }
}
...
abstract class K_Model  {
  ...
  function retrieve($pkvalue) {
    ...
    echo (SHOW_QUERY==1 ? $sql.'---'.$pkvalue.'<br>' : '');
    $stmt = $dbh->prepare($sql);
  ...
  function query($sql,$arguments=array()){
        $dbh=$this->getdbh();
	$result='';
	foreach($arguments as $arg => $val){
	$result.='--'.$val;
	}
	 echo (SHOW_QUERY==1 ? $sql.'---'.$result.'<br>' : '');
	try {
	$stmt = $dbh->prepare($sql);
...
/////////////////////// update query 
 function updatequery($query,$arguments=array())
 {
  ...
  $dbh=$this->getdbh();
	...
	$result.='--'.$val;
	 ...
	 echo (SHOW_QUERY==1 ? $query.'---'.$result.'<br>' : '');
	try{
...
function deletequery($query,$args=array())
{
$dbh=$this->getdbh();


	echo (SHOW_QUERY==1 ? $query.'---'.$args.'<br>' : '');
